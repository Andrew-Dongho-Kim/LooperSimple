...
class AppWidgetUpdateWorker @AssistedInject constructor(
    ...
    private suspend fun done() = respond(DoneState.DONE)
    private suspend fun skip() = respond(DoneState.SKIP)

    private suspend fun respond(state: Int) =
        mutations.respondIfUnanswered(loopId(), requestedDate(), state)

    private suspend fun start() = mutations.start(loopId(), requestedDate())
    private suspend fun stop() = mutations.stop(loopId(), requestedDate())
