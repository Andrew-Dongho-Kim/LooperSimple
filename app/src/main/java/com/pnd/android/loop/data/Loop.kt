...
data class Loop(
    ...
    companion object {
        fun popFromIntent(intent: Intent): Loop {
            return Loop(
...
fun Loop.description(context: Context) =
    """ -->
    |*Loop  
    | title : $title
    ...
    | enabledDays : ${Loop.Day.description(loopEnableDays)}
...
interface LoopDao {
    ...
    fun getAll(): LiveData<List<Loop>>

    @Query("SELECT * FROM loop")
    suspend fun syncGetAll(): List<Loop>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun add(vararg loops: Loop): List<Long>
